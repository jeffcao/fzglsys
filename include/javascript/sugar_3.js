...
function checkMinSupported(c, s) {
    ...
    for (var i in supported) {
        if (current[i] && parseInt(current[i]) > parseInt(supported[i]))return true; else if (current[i] && parseInt(current[i]) < parseInt(supported[i]))return false;
    }
    return true;
}
function checkMaxSupported(c, s) {
    ...
    var supported = s.split(".");
    for (var i in supported) {
        if (current[i] && parseInt(current[i]) > parseInt(supported[i]))return false; else if (current[i] && parseInt(current[i]) < parseInt(supported[i]))return true;
    }
    return true;
}
SUGAR.isSupportedBrowser = function () {
    var supportedBrowsers = {msie: {min: 9, max: 10}, safari: {min: 534}, mozilla: {min: 31.0}, chrome: {min: 37}};
...
function validate_form(formname, startsWith) {
    ...
    if (isError == true) {
        ...
        for (var wp = 0; wp < inputsWithErrors.length; wp++) {
            ...
            if (!(elementCoor.x >= nwX && elementCoor.y >= nwY && elementCoor.x <= seX + nwX && elementCoor.y <= seY + nwY)) {
                inView = false;
...
function parseDate(input, format) {
    ...
    if (date.match(/^\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}$/) && yAt == 4) {
        ...
        digits = 0;
        if (date.match(/^\d{8}$/))digits = 8; else if (date.match(/\d{6}/))digits = 6; else if (date.match(/\d{4}/))digits = 4; else if (date.match(/\d{5}/))digits = 5;
        switch (yAt) {
            case 0:
                switch (digits) {
                    case 4:
...
function onUnloadEditView(theForm) {
    ...
    if (typeof theForm == 'undefined') {
        for (var idx in editViewSnapshots) {
            ...
            if (theForm == null || typeof editViewSnapshots[theForm.id] == 'undefined' || editViewSnapshots[theForm.id] == null || !SUGAR.loadedForms[theForm.id]) {
                continue;
            }
...
sugarListView.prototype.use_external_mail_client = function (no_record_txt, module) {
    ...
    YAHOO.util.Connect.asyncRequest("POST", "index.php?", {success: this.use_external_mail_client_callback}, SUGAR.util.paramsToUrl({
        module: "Emails",
        action: "Compose",
...
sugarListView.prototype.send_form_for_emails = function (select, currentModule, action, no_record_txt, action_module, totalCount, totalCountError) {
    ...
    var callback = {
        success: function (o) {
            ...
            quickComposePackage.fullComposeUrl = 'index.php?module=Emails&action=Compose&ListView=true' + '&uid=' + uidTa.value + '&action_module=' + action_module;
    ...
    switch (mode) {
        case'selected':
            for (var wp = 0; wp < document.MassUpdate.elements.length; wp++) {
                ...
                if (typeof document.MassUpdate.elements[wp].name != 'undefined' && document.MassUpdate.elements[wp].name == 'mass[]' && document.MassUpdate.elements[wp].checked && !reg_for_existing_uid.test(document.MassUpdate.uid.value)) {
    ...
    return {
        getAndRemove: function (el) {
                        ...
                        if (r1.test(srcResult) && srcResult.indexOf(window.location.hostname) == -1) {
                            YUI().use('get', function (Y) {
                                ...
                                Y.Get.script(srcResult, {
                                    autopurge: false, onSuccess: function (o) {
            ...
            $dialog.html(helpText).dialog({
                ...
                position: {my: myPos, at: atPos, of: $(el)}
            });
            ...
            var $dialog = $('<div class="open"></div>').html(body).dialog({
                autoOpen: false,
                title: caption,
                width: 300,
                position: {my: 'right top', at: 'left top', of: $(el)}
            });
            if (show_buttons) {
            ...
            this._doWhenStack.push({check: condition, fn: fn, obj: params, overrideContext: scope});
        ...
        }, _doWhenStack: [], _doWhenInterval: false, _doWhenCheck: function () {
        ...
        searchFormSelect: function (view, previousView) {
            ...
            for (var i = 0; i < elemList.length; i++) {
                ...
                if (elemType == 'text' || elemType == 'textarea' || elemType == 'password') {
                    ...
                    var optionList = elem.options, selectedIndex = 0;
                    for (var ii = 0; ii < optionList.length; ii++) {
                    ...
                    if (elem.name.indexOf("_id") != -1 || elem.name.indexOf("_c") != -1 || elem.name.indexOf("_advanced") != -1) {
                        elem.value = '';
                    }
                }
            }
...
SUGAR.tabChooser = function () {
    ...
    return {
        frozenOptions: [], movementCallback: function (left_side, right_side) {
...
SUGAR.language = function () {
    return {
        languages: new Array(), setLanguage: function (module, data) {
...
SUGAR.contextMenu = function () {
    return {
        objects: new Object(), objectTypes: new Object(), registerObject: function (objectType, id, metaData) {
...
SUGAR.contextMenu.actions = function () {
    return {
        createNote: function (itemClicked, metaData) {
            ...
            for (i in metaData) {
                if (i == 'notes_parent_type')loc += '&parent_type=' + metaData[i]; else if (i != 'module' && i != 'parent_type')loc += '&' + i + '=' + metaData[i];
            }
...
SUGAR.append(SUGAR.util, {
    isTouchScreen: function () {
        ...
        show: function (module, id, new_status, viewType, parentContainerId) {
            ...
            SUGAR.util.closeActivityPanel.panel = new YAHOO.widget.SimpleDialog("closeActivityDialog", {
                width: "300px",
                fixedcenter: true,
                visible: false,
                draggable: false,
                ...
                buttons: [{
                    text: SUGAR.language.get("app_strings", "LBL_EMAIL_OK"), handler: function () {
                        ...
                        var callback = {
                            success: function (o) {
                                window.setTimeout(function () {
                                    if (document.getElementById('search_form'))document.getElementById('search_form').submit(); else window.location.reload(true);
                                }, 0);
...
SUGAR.MultiEnumAutoComplete.getMultiSelectValuesFromKeys = function (options_index, val_string) {
    var opts = SUGAR.language.get('app_list_strings', options_index);
    val_string = val_string.replace(/^\^/, '').replace(/\^$/, '')
    var selected_values = val_string.split("^,^");
    if (selected_values.length > 0 && selected_values.indexOf('') == selected_values.length - 1) {
        selected_values.pop();
    }

...
class wfm_utils {
	...
	static $wfm_release_version = '5.1';
	static $wfm_code_version = 'D20150121T1636'; // php, js, css, ...
	static $wfm_db_structure_version = 3;
	static $wfm_workflows_version = 4;
	...
	static function wfm_echo($type, $text) {
		...
		switch ($type) {
			case 'crontab':
				if ($request['execution_type'] != "crontab") {
					break;
				}
	...
	static function wfm_generate_field_input($field_name, $field_value, $size=30) {
		return "<input type='text' accesskey='7' title='' value='{$field_value}' maxlength='255' size='{$size}' id='{$field_name}' name='{$field_name}'>";
	}
	...
	static function wfm_add_jsModLanguages($trigger_module, $add_related_modules, $add_id_relationships, $related_modules, $focus, $bean, $fieldsToBeRemoved, $translateFieldLabels, $rhs_key) {
		self::wfm_log('asol_debug', 'get_defined_vars=['.print_r(get_defined_vars(), true).']', __FILE__, __METHOD__, __LINE__);
	...
	static public function cleanWFMBrokenWorkingNodes() {
		...
		$db->query("
			...
			WHERE status = 'executing' AND date_modified < now() - INTERVAL 1 HOUR;
		");
	}

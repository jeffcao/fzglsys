...
// BEGIN - Compatibility sugarcrm 7.5
	...
	/*
	 	// If Not $current_user->getSystemUser(); 
	 	// function retrieve does not work
	 	SELECT 
		email_templates.* ,
		users_au.first_name au_first_name,
		users_au.last_name au_last_name,
		teams_tn.name as tn_name,
		teams_tn.name_2 as tn_name_2 
		FROM email_templates  
		LEFT JOIN users users_au ON users_au.id = email_templates.assigned_user_id 
		LEFT JOIN teams teams_tn ON teams_tn.id = email_templates.team_id  
		WHERE email_templates.id = '79fabbc7-5e00-b760-4486-54bcfc88d500' AND email_templates.deleted=0 
		AND email_templates.team_set_id IN 
			(
				select tst.team_set_id 
				from team_sets_teams tst
		        INNER JOIN team_memberships team_memberships ON tst.team_id = team_memberships.team_id AND team_memberships.user_id = '' AND team_memberships.deleted = 0
			)  LIMIT 0,1
			
		// If $current_user->getSystemUser(); 
		// function retrieve works as expected
		SELECT 
		email_templates.* ,
		users_au.first_name au_first_name,
		users_au.last_name au_last_name,
		...
		LEFT JOIN users users_au ON users_au.id = email_templates.assigned_user_id 
		LEFT JOIN teams teams_tn ON teams_tn.id = email_templates.team_id  
		WHERE email_templates.id = '79fabbc7-5e00-b760-4486-54bcfc88d500' AND email_templates.deleted=0 LIMIT 0,1
	 */
	
	// END - Compatibility sugarcrm 7.5

			...
			foreach ($value_parent_event as $key_activity => $value_activity) {
				...
				if (!in_array($value_activity['id'], $activity_ids_event_duplicity)) {	// Event duplicity.
					...
					foreach($next_activity_ids_all_tree as $key => $value) {
						...
						wfm_utils::wfm_log('flow_debug', "\$value".print_r($value,true), __FILE__, __METHOD__, __LINE__);
						...
						foreach ($export_array['tasks'] as $key_parent_activity => $value_parent_activity) {
							if ($key_parent_activity == $value_activity['id']) {
								...
								foreach ($value_parent_activity as $key_task => $value_task) {
									$draw_Tasks_of_this_activity .= generate_Task_HTML('workflow', $value_task['id'], $value_task['name'], $value_task['description'], $value_task['task_type'], null, $bottom_Tasks_of_this_activity, $xCoordinateTasks, $value_task['delay_type'], $value_task['delay'], $value_task['task_order'], $value_task['task_implementation']);
...
function generate_Process_HTML($id, $name, $alternative_database, $trigger_module, $status, $description) {
	...
	$top = 10;
	$left = 10;
	$qtip_info = generate_process_info_HTML($alternative_database, $trigger_module, $status, $description);
...
function generate_Activity_HTML($id, $name, $description, $top, $left, $width, $draw_Tasks_of_this_activity, $counter_Tasks_of_this_activity, $activity_conditions, $delay, $module, $relationship) {
	...
	return "
		<div module='{$wfm_module}' id='{$id}' relationship='{$relationship}' class='{$wfm_module} jstree-drop' style='top:{$top}px; left:{$left}px; width:auto;'>
...
function generate_Task_HTML($generateTaskHtmlType, $id, $name, $description, $task_type, $top, $bottom, $left, $delay_type, $delay, $order, $task_implementation) {
	...
	if ($task_type == 'call_process') {
		...
		$id_event = $task_implementation_array[0];
		$name_event = $task_implementation_array[1];
		...
		$draw_Call_process_open_subprocess .= "
			<span class='task_call_process_open_subprocess_icon'>
				<a class='redirectLink' link_module='asol_Events' link_record='{$id_event}' onclick='' qtip_info='{$subprocess_info}'>
	...
	$qtip_info = generate_info_for_Task_HTML($name, $description, $delay_type, $order);
...
function generateFlowChart($processId) {
	...
	if (!empty($export_array['processes'])) {
		$workflowHeader = generate_Process_HTML($export_array['processes'][0]['id'], $export_array['processes'][0]['name'], $export_array['processes'][0]['alternative_database'],$export_array['processes'][0]['trigger_module'],  $export_array['processes'][0]['status'], $export_array['processes'][0]['description']);
	}
...
";
	...
	foreach ($tasks as $keyTask => $task) {
		$drawTasks .= generate_Task_HTML('recycleBinTasks', $task['id'], $task['name'], $task['description'], $task['task_type'], $top, null, $left, $task['delay_type'], $task['delay'], $task['order'], $task['task_implementation']);

...
var blockedByLoadFlowChart = false;
var blockedByLoadObjectEditor = false;
...
function loadSeveralFeaturesLayout() {
	...
	$("#objectEditor").load(function() {
		...
		if (blockedByLoadFlowChart == false) {
			unblockUI();
		}
		blockedByLoadObjectEditor = false;
	});
...
function loadObjectEditor(urlForIframeSrc) {
	...
	blockedByLoadObjectEditor = true;
	blockUI_type('load');
...
function loadFlowChart(block) {
	...
	try {
		...
		$.ajax({
			...
			complete : function(response) {
				if (blockedByLoadObjectEditor == false) {
					unblockUI();
				}
				blockedByLoadFlowChart = false;
			}
		});
	} catch (exception) {
		alert(exception);
...
function saveObjectAndReload(editViewForm) {
	console.log('saveObjectAndReload');
	console.dir(arguments);
	...
	$("#workflow, #recycleBin").contextmenu({
		...
		menu : [ {
			...
			action : function(event, ui) {
				...
				console.log(validTargetNodeForContextMenu);

...
if ($return_action == "restoreCSS")	{
	...
	copy(getcwd()."/modules/asol_Reports/include_basic/css/reports_original.css", getcwd()."/modules/asol_Reports/include_basic/css/reports.css");
	
	$userDomain = (isset($current_user->asol_default_domain) ? $current_user->asol_default_domain : null);
	if (($userDomain !== null) && (is_file("modules/asol_Reports/include_basic/css/".$userDomain.".css"))) {
		@unlink("modules/asol_Reports/include_basic/css/".$userDomain.".css");
	}
	...
	if ($name != "") {
		...
		$target =  (!empty($current_user->asol_default_domain)) ? getcwd()."/modules/asol_Reports/include_basic/css/".$current_user->asol_default_domain.".css" : getcwd()."/modules/asol_Reports/include_basic/css/reports.css";
		copy($tmpName, $target);

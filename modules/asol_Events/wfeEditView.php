<?php 
require_once("modules/asol_Process/___common_WFM/php/asol_utils.php");
wfm_utils::wfm_log('debug', "ENTRY", __FILE__);

wfm_utils::wfm_log('debug', '$_REQUEST=['.var_export($_REQUEST, true).']', __FILE__, __METHOD__, __LINE__);

// FIXME sugarcrm 7.5
//require_once('include/Smarty/plugins/function.sugar_help.php');
//$sugar_smarty = new Sugar_Smarty();

global $mod_strings, $app_list_strings, $app_strings;

$focus = new asol_Events();
$focusId = (isset($_REQUEST['record'])) ? $_REQUEST['record'] : '';

if (!empty($focusId)) { // Modify
	$focus->retrieve($focusId);
	
} else { // Create
	
}

// Get sugarcrm Theme
$themeObject = SugarThemeRegistry::current();
$css = $themeObject->getCSS();
$js = $themeObject->getJS();

?>

<html>

	<head>
	
		<script src="modules/asol_Process/___common_WFM/plugins_js_css_images/jsLab/LAB.min.js?version=<?php wfm_utils::echoVersionWFM(); ?>" type="text/javascript"></script>
	
		<script>
			var viewType = 'wfeEditView';
		</script>
	
		<!-- BEGIN - SUGARCRM JS -->
			<?php 
				if (!is_file(sugar_cached("include/javascript/sugar_grp1.js")) || !is_file(sugar_cached("include/javascript/sugar_grp1_yui.js")) || !is_file(sugar_cached("include/javascript/sugar_grp1_jquery.js"))) {
					$_REQUEST['root_directory'] = ".";
					require_once("jssource/minify_utils.php");
					ConcatenateFiles(".");
				}
				echo getVersionedScript('cache/include/javascript/sugar_grp1_jquery.js');
				echo getVersionedScript('cache/include/javascript/sugar_grp1_yui.js');
				echo getVersionedScript('cache/include/javascript/sugar_grp1.js');
				
				echo wfm_utils::_getModLanguageJS('asol_Events');
				echo wfm_utils::getAppStringsCache();
			?>
		<!-- END - SUGARCRM JS -->
		
		<script>
			if (typeof jQuery === "undefined") {
				$LAB.script("modules/asol_Process/___common_WFM/js/jquery.min.js");
			}
		</script>
	
		<!-- BEGIN - SUGARCRM THEME -->
			<?php echo $css; ?>
			<?php echo $js; ?>
		<!-- END - SUGARCRM THEME -->
		
		<script src="modules/asol_Events/js/asol_events.js?version=<?php wfm_utils::echoVersionWFM(); ?>"></script>
		<script src="modules/asol_Events/js/module_fields.js?version=<?php wfm_utils::echoVersionWFM(); ?>" ></script>
		<script src="modules/asol_Process/___common_WFM/js/common_event_activity_task.js?version=<?php wfm_utils::echoVersionWFM(); ?>" type="text/javascript"></script>
		
		<link href="modules/asol_Process/_flowChart/wfeEditView.css?version=<?php wfm_utils::echoVersionWFM(); ?>" rel="stylesheet" type="text/css" />
		<script	src='modules/asol_Process/_flowChart/wfeEditView.js?version=<?php wfm_utils::echoVersionWFM(); ?>'></script>
		
		<link href="modules/asol_Events/css/asol_events_style.css?version=<?php wfm_utils::echoVersionWFM(); ?>" rel="stylesheet" type="text/css" />
		
	</head>
	
	<body>
		<table style="width:100%">
			<tbody>
				<tr>
					<td>
					<div class='moduleTitle'>
							<h2><span class="pointer">»</span><?php echo $app_strings['LBL_EDIT_BUTTON_LABEL']; ?></h2>
						</div>
						<script>
							/*
						    $(document).ready(function() {
						        $("ul.clickMenu").each(function(index, node) {
						            $(node).sugarActionMenu();
						        });
						    });
						    */
						</script>
						<div class="clear"></div>
						<form action="" method="POST" name="EditView" id="EditView" >
							<table width="100%" cellpadding="0" cellspacing="0" border="0" class="dcQuickEdit">
								<tr>
									<td class="buttons">
										<input type="hidden" name="module" value="asol_Events">
										<input type="hidden" name="record" value="<?php echo $focus->id; ?>">
										<input type="hidden" name="isDuplicate" value="false">
										<input type="hidden" name="action">
										<input type="hidden" name="return_module" value="">
										<input type="hidden" name="return_action" value="">
										<input type="hidden" name="return_id" value="">
										<input type="hidden" name="module_tab">
										<input type="hidden" name="contact_role">
										<input type="hidden" name="offset" value="1">
										<!-- to be used for id for buttons with custom code in def files-->
										<div class="action_buttons">
											<input title="<?php echo $app_strings['LBL_SAVE_BUTTON_TITLE']; ?>" accessKey="a" class="button primary" onclick="" type="submit" name="button" value="<?php echo $app_strings['LBL_SAVE_BUTTON_LABEL']; ?>" id="SAVE_HEADER">
										</div>
									</td>
									<td align='right'></td>
								</tr>
							</table>
							<span id='tabcounterJS'>
								<script>
									/*
						            SUGAR.TabFields = new Array();
						            //this will be used to track tabindexes for references
						            */
								</script>
							</span>
							<div id="EditView_tabs">
								<div >
									<div id="detailpanel_1" >
										<table width="100%" border="0" cellspacing="1" cellpadding="0"  id='Default_asol_Events_Subpanel'  class="yui3-skin-sam edit view panelContainer">
											<tr>
												<td width="5%" valign="top" scope="col" id="name_label"> <?php echo $mod_strings['LBL_NAME']; ?>: <span class="required">*</span></td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/name.php"); ?>
												</td>
											</tr>
											<tr>
											
												<td width="5%" valign="top" scope="col" id="trigger_type_label">
													<?php echo $mod_strings['LBL_TRIGGER_TYPE']; ?>:<?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_TRIGGER_TYPE']),$sugar_smarty); ?> 
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/trigger_type.php"); ?>
												</td>
												
												<td width="5%" valign="top" scope="col" id="type_label">
													<?php echo $mod_strings['LBL_TYPE']; ?>:<?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_TYPE']),$sugar_smarty); ?> 
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/type.php"); ?>
												</td>
												
												<td width="5%" valign="top" scope="col" id="scheduled_type_label">
													<?php echo $mod_strings['LBL_SCHEDULED_TYPE']; ?>:<?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_SCHEDULED_TYPE']),$sugar_smarty); ?> 
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/scheduled_type.php"); ?>
												</td>
												
												<td width="5%" valign="top" scope="col" id="subprocess_type_label">
													<?php echo $mod_strings['LBL_SUBPROCESS_TYPE']; ?>:<?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_SUBPROCESS_TYPE']),$sugar_smarty); ?> 
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/subprocess_type.php"); ?>
												</td>
												
											</tr>
											<tr>
												<td width="5%" valign="top" scope="col" id="description_label"> <?php echo $mod_strings['LBL_DESCRIPTION']; ?>: </td>
												<td width="95%" valign="top" colspan="10">
													<?php require_once("modules/asol_Process/customFields/description.php"); ?>
												</td>
											</tr>
										</table>
									</div>
									<div id="detailpanel_2" class="edit view edit508">
										<h4>&nbsp;&nbsp; <a href="javascript:void(0)" class="collapseLink" onclick="collapsePanel(2);"> <img border="0" id="detailpanel_2_img_hide" src="modules/asol_Process/___common_WFM/images/basic_search.gif?v=ttn-4I-G5zA_ijZVX7YRVg"></a><a href="javascript:void(0)" class="expandLink" onclick="expandPanel(2);"> <img border="0" id="detailpanel_2_img_show" src="modules/asol_Process/___common_WFM/images/advanced_search.gif?v=ttn-4I-G5zA_ijZVX7YRVg"></a> <?php echo translate('LBL_ASOL_TRIGGERING_PANEL', 'asol_Events'); ?>
										<script>
						                    document.getElementById('detailpanel_2').className += ' expanded';
										</script></h4>
										<table width="100%" border="0" cellspacing="1" cellpadding="0"  id='LBL_ASOL_TRIGGERING_PANEL'  class="yui3-skin-sam edit view panelContainer">
											<tr>
											
												<td width="5%" valign="top" scope="col" id="scheduled_tasks_label">
													<?php echo $mod_strings['LBL_SCHEDULED_TASKS']; ?>:<?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_SCHEDULED_TASKS']),$sugar_smarty); ?> 
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/scheduled_tasks.php"); ?>
												</td>
												
											</tr>
											<tr>
												<td width="5%" valign="top" scope="col" id="trigger_module_label"> <?php echo $mod_strings['LBL_ASOL_TRIGGER_MODULE']; ?>: </td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/trigger_module.php"); ?>
												</td>
											
												<td width="5%" valign="top" scope="col" id="trigger_event_label"> 
													<?php echo $mod_strings['LBL_ASOL_TRIGGER_EVENT']; ?>: <?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_TRIGGER_EVENT']),$sugar_smarty); ?>
												</td>
												<td width="75%" valign="top">
													<?php require_once("modules/asol_Events/customFields/trigger_event.php"); ?>
												</td>
												
											</tr>
										</table>
										<script type="text/javascript">
						                    SUGAR.util.doWhen("typeof initPanel == 'function'", function() {
						                        initPanel(2, 'expanded');
						                    });
										</script>
									</div>
									<div id="detailpanel_3" class="edit view edit508">
										<h4>&nbsp;&nbsp; <a href="javascript:void(0)" class="collapseLink" onclick="collapsePanel(3);"> <img border="0" id="detailpanel_3_img_hide" src="modules/asol_Process/___common_WFM/images/basic_search.gif?v=ttn-4I-G5zA_ijZVX7YRVg"></a><a href="javascript:void(0)" class="expandLink" onclick="expandPanel(3);"> <img border="0" id="detailpanel_3_img_show" src="modules/asol_Process/___common_WFM/images/advanced_search.gif?v=ttn-4I-G5zA_ijZVX7YRVg"></a> <?php echo translate('LBL_ASOL_EVENT_CONDITION_PANEL', 'asol_Events'); ?>
										<script>
						                    document.getElementById('detailpanel_3').className += ' expanded';
										</script></h4>
										<table width="100%" border="0" cellspacing="1" cellpadding="0"  id='LBL_ASOL_EVENT_CONDITION_PANEL'  class="yui3-skin-sam edit view panelContainer">
											<tr>
											
												<td width="5%" valign="top" scope="col" id="module_fields_label"> 
													<?php echo $mod_strings['LBL_ASOL_MODULE_FIELDS']; ?>: <?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_MODULE_FIELDS']),$sugar_smarty); ?>
												</td>
												<td width="15%" valign="top">
													<?php require_once("modules/asol_Events/customFields/module_fields.php"); ?>
												</td>
												
												<td width="5%" valign="top" scope="col" id="module_conditions_label"> 
													<?php echo $mod_strings['LBL_CONDITIONS']; ?>: <?php echo wfm_utils::smarty_function_sugar_help(array("text"=>$mod_strings['LBL_POPUPHELP_FOR_FIELD_MODULE_CONDITIONS']),$sugar_smarty); ?>
												</td>
												<td width="75%" valign="top">
													<?php require_once("modules/asol_Events/customFields/module_conditions.php"); ?>
												</td>
											
											</tr>
										</table>
										<script type="text/javascript">
						                    SUGAR.util.doWhen("typeof initPanel == 'function'", function() {
						                        initPanel(3, 'expanded');
						                    });
										</script>
									</div>
								</div>
							</div>
							<script language="javascript">
								/*
						        var _form_id = 'EditView';
						
						        SUGAR.util.doWhen(function() {
						            _form_id = (_form_id == '') ? 'EditView' : _form_id;
						            return document.getElementById(_form_id) != null;
						        }, SUGAR.themes.actionMenu);
						        */
						
							</script>
							<!-- to be used for id for buttons with custom code in def files-->
							<div class="buttons">
								<div class="action_buttons">
									<input title="<?php echo $app_strings['LBL_SAVE_BUTTON_TITLE']; ?>" accessKey="a" class="button primary" onclick="" type="submit" name="button" value="<?php echo $app_strings['LBL_SAVE_BUTTON_LABEL']; ?>" id="SAVE_FOOTER">
								</div>
							</div>
						</form>
						<script type="text/javascript" language="JavaScript"></script>
						<script>
							/*
						    SUGAR.util.doWhen("document.getElementById('EditView') != null", function() {
						        SUGAR.util.buildAccessKeyLabels();
						    }); 
						    */
						</script>
						<script type="text/javascript">
							/*
						    YAHOO.util.Event.onContentReady("EditView", function() {
						        initEditView(document.forms.EditView)
						    });
						    //window.setTimeout(, 100);
						    window.onbeforeunload = function() {
						        return onUnloadEditView();
						    };
						    // bug 55468 -- IE is too aggressive with onUnload event
						    if ($.browser.msie) {
						        $(document).ready(function() {
						            $(".collapseLink,.expandLink").click(function(e) {
						                e.preventDefault();
						            });
						        });
						    }
						    */
						</script>
						<script type="text/javascript">
						    addForm('EditView');
						    addToValidate('EditView', 'name', 'name', true, 'Name');
						    /*
						    addToValidate('EditView', 'date_entered_date', 'date', false, 'Date Created');
						    addToValidate('EditView', 'date_modified_date', 'date', false, 'Date Modified');
						    addToValidate('EditView', 'modified_user_id', 'assigned_user_name', false, 'Modified By');
						    addToValidate('EditView', 'modified_by_name', 'relate', false, 'Modified By');
						    addToValidate('EditView', 'created_by', 'assigned_user_name', false, 'Created By');
						    addToValidate('EditView', 'created_by_name', 'relate', false, 'Created By');
						    addToValidate('EditView', 'description', 'text', false, 'Description');
						    addToValidate('EditView', 'deleted', 'bool', false, 'Deleted');
						    addToValidate('EditView', 'assigned_user_id', 'relate', false, 'Assigned User Id');
						    addToValidate('EditView', 'assigned_user_name', 'relate', false, 'User');
						    addToValidate('EditView', 'type', 'enum', false, 'Type');
						    addToValidate('EditView', 'trigger_type', 'enum', false, 'Trigger Type');
						    addToValidate('EditView', 'trigger_event', 'enum', false, 'Trigger Event');
						    addToValidate('EditView', 'conditions', 'text', false, 'Conditions');
						    addToValidate('EditView', 'scheduled_tasks', 'text', false, 'Scheduled Tasks');
						    addToValidate('EditView', 'scheduled_type', 'enum', false, 'Scheduled Type');
						    addToValidate('EditView', 'subprocess_type', 'enum', false, 'Subprocess Type');
						    addToValidate('EditView', 'module_fields', 'varchar', false, 'Module Fields');
						    addToValidate('EditView', 'module_conditions', 'varchar', false, 'Conditions');
						    addToValidate('EditView', 'trigger_module', 'varchar', false, 'Trigger Module');
						    addToValidate('EditView', 'asol_process_asol_events_1_name', 'relate', false, 'Process');
						    addToValidate('EditView', 'asol_process_asol_events_name', 'relate', false, 'Process');
						    addToValidateBinaryDependency('EditView', 'assigned_user_name', 'alpha', false, 'No match for field: Assigned to', 'assigned_user_id');
						    */
						</script>
						<br>
						<span style="white-space:nowrap;"> <a onClick="openWFMVariableGenerator(100, 100);" style="cursor: pointer;"> <img border="0" align="absmiddle" alt="<?php echo translate('LBL_WFM_VARIABLE_GENERATOR_BUTTON', 'asol_Process'); ?>" src="modules/asol_Process/___common_WFM/images/wfm_variable_generator.png"> <span><?php echo translate('LBL_WFM_VARIABLE_GENERATOR_BUTTON', 'asol_Process'); ?></span> </a> </span>
						
						<script>
						    function openWFMVariableGenerator(left, top) {
						        var popup = window.open('index.php?entryPoint=wfm_variable_generator', 'wfm_variables', 'width=1000, height=750, top=' + top + ', left=' + left + ', location=no, menubar=no, resizable=yes, scrollbars=yes, status=no, titlebar=no, toolbar=no');
						        popup.focus();
						    }
						</script>
						<script>
							/*
						    secondsSinceLoad = 0;
						    alertList = [];
						    if (!alertsTimeoutId) {
						        checkAlerts();
						    }
						    */
						</script>
					</td>
				</tr>
			</tbody>
		</table>
		
	</body>
	
</html>

...
function task_call_process($task_implementation, $alternative_database, $parent_trigger_module, $parent_bean_id, $parent_process_instance_id, $bean_ungreedy_count, $old_bean, $new_bean, $custom_variables, $current_user_id) {
	...
	$event_id = $task_implementation_array[0];
	$event_name = $task_implementation_array[1];
	$execute_subprocess_immediately = ($task_implementation_array[2] === 'true') ? true : false;
...
function task_add_custom_variables($task_implementation, $alternative_database, $trigger_module, $bean_id, $old_bean, $new_bean, & $custom_variables, $current_user_id) {
		...
		switch ($type) {
			case 'sql':
				...
				if (!empty($moduleField_array_value)) {
					...
					wfm_utils::wfm_log('asol_debug', '$sql=['.print_r($sql, true).']', __FILE__, __METHOD__, __LINE__);
					...
					$sql = replace_wfm_vars('custom_variable_sql', html_entity_decode($sql), $alternative_database, $trigger_module, $bean_id, $old_bean, $new_bean, $custom_variables, $current_user_id);
	...
	wfm_utils::wfm_log('asol_debug', '(* user_defined)$custom_variables=['.var_export($aux_custom_variables, true).']', __FILE__, __METHOD__, __LINE__);
	...
	foreach($beanItems as $beanItem) {
		...
		switch (count($beanValues)) {
				...
				switch ($beanValues[0]) {
					...
					case 'old_bean':
						...
						if ($alternative_database == -1) {
							...
							} else if ((($fieldNameMap[$beanValues[1]]['type'] == 'datetime') || ($fieldNameMap[$beanValues[1]]['dbType'] == 'datetime')) || (($fieldNameMap[$beanValues[1]]['type'] == 'datetimecombo') || ($fieldNameMap[$beanValues[1]]['dbType'] == 'datetime')) || (($fieldNameMap[$beanValues[1]]['type'] == 'datetimecombo') || ($fieldNameMap[$beanValues[1]]['dbType'] == 'datetimecombo'))) {
			...
			case 4:
				...
				switch ($beanValues[0]) {
					case 'c_var':
						$beanField = $custom_variables[$beanValues[1]][$beanValues[2]][$beanValues[3]];
						break;
				}

...
class M2MRelationship extends SugarRelationship
{
    ...
    public function getSubpanelQuery($link, $params = array(), $return_array = false)
    {
        ...
        // ASOL
        $asol_selfReferencing = ($link->relationship->getLHSModule() == $link->relationship->getRHSModule());
    	if (!$asol_selfReferencing) {
        	$where = "$startingTable.$startingKey=$joinTable.$startingJoinKey AND $joinTable.$joinKey='{$link->getFocus()->$targetKey}'";
        } else {
        	$where = "$startingTable.$startingKey=$joinTable.$joinKey AND $joinTable.$startingJoinKey='{$link->getFocus()->$targetKey}'";
        }
        // ASOL

<?php
/***CONFIGURATOR***/
$sugar_config['email_xss'] = '';
$sugar_config['WFM_AlternativeDbConnections']['db_host_name'] = 'localhost';
$sugar_config['WFM_AlternativeDbConnections']['db_host_instance'] = 'SQLEXPRESS';
$sugar_config['WFM_AlternativeDbConnections']['db_user_name'] = 'root';
$sugar_config['WFM_AlternativeDbConnections']['db_password'] = 'MyRoot123';
$sugar_config['WFM_AlternativeDbConnections']['db_name'] = 'fzglsys';
$sugar_config['WFM_AlternativeDbConnections']['db_type'] = 'mysql';
$sugar_config['WFM_AlternativeDbConnections']['db_port'] = '';
$sugar_config['WFM_AlternativeDbConnections']['db_manager'] = 'MysqliManager';
$sugar_config['disabled_languages'] = '';
$sugar_config['default_language'] = 'zh_cn';
/***CONFIGURATOR***/
...
class SugarFieldFile extends SugarFieldBase {
    ...
    public function save(&$bean, $params, $field, $vardef, $prefix = ''){
        ...
        $prefix_stored_name = str_replace("filename","",$field);
		...
		if (isset($_FILES[$prefix . $field . '_file']) && $upload_file->confirm_upload())
		{
    		...
    		$bean->$real_file_mine_type = $upload_file->mime_type;
			$bean->$real_file_ext = $upload_file->file_ext;
			$move=true;
		}
        ...
        $real_stored_file_name = $prefix_stored_name.$bean->id;
		if ($move) {
            $upload_file->final_move($real_stored_file_name);
            $upload_file->upload_doc($bean, $real_stored_file_name, $params[$prefix . $vardef['docType']], $bean->$field, $upload_file->mime_type);
            ...
            if ( isset($params[$prefix . $vardef['docId']])
                ...
                if(!empty($extension))
                {
                	$bean->$real_file_ext = $extension;
                	$bean->$real_file_mine_type = get_mime_content_type_from_filename($bean->$field);
                }
            }
        }
